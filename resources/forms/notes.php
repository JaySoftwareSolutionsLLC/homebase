	...
	<script>
		$(document).ready(function() {
			...
			$('select.type').on('change', function() {
				...
				let completedDatetimeEl = $('input#complete-datetime-input');
				let estMinToCompEl = $('input#est-min-to-comp-input');
				if (textAreaEl.val().length === 0) {
					switch (newVal) {
						...
						case 'learning resource':
							...
							let currDateTime = new Date($.now());
							completedDatetimeEl.setNow(); // Custom function defined in resources.js
							estMinToCompEl.css('background', 'yellow');
							break;
						default:
							break;
					}
