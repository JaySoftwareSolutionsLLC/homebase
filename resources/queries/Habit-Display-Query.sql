SELECT 	h.id
        ...
        ,(SELECT COUNT(0) 
          ...
          WHERE ((`h`.`id` = `hl`.`habit_id`) and (`hl`.`status` = 'Completed') and (case when ((`hm`.`frequency_window` = 'Day') and (dayofmonth(`hl`.`datetime`) = dayofmonth(now())) and (month(`hl`.`datetime`) = month(now())) and (year(`hl`.`datetime`) = year(now()))) then 1 when ((`hm`.`frequency_window` = 'Week') and (week(`hl`.`datetime`,3) = week(now(),3)) and (year(`hl`.`datetime`) = year(now()))) then 1 when ((`hm`.`frequency_window` = 'Month') and (month(`hl`.`datetime`) = month(now())) and (year(`hl`.`datetime`) = year(now()))) then 1 when ((`hm`.`frequency_window` = 'Quarter') and (quarter(`hl`.`datetime`) = quarter(now())) and (year(`hl`.`datetime`) = year(now()))) then 1 when ((`hm`.`frequency_window` = 'Year') and (year(`hl`.`datetime`) = year(now()))) then 1 else 0 end))) AS `completed`
      	,(SELECT count(0)
          FROM `jaysoftw_homebase`.`personal_wellness_habit_logs` `hl` 
          WHERE ((`h`.`id` = `hl`.`habit_id`) 
            and (`hl`.`status` = 'Started')
            AND (DATE(`hl`.`datetime`) = DATE(NOW())) )) AS `started`
        ,(select count(0) 
          from `jaysoftw_homebase`.`personal_wellness_habit_logs` `hl` 
          where ((`hl`.`status` = 'Completed') 
                and (`h`.`id` = `hl`.`habit_id`) 
                and (month(`hl`.`datetime`) = month(now())) 
                and (dayofmonth(`hl`.`datetime`) = dayofmonth(now())))) AS `logged_today`
