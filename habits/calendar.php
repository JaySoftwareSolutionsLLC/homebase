<?php
    ...
    include($_SERVER["DOCUMENT_ROOT"] . '/homebase/habits/resources/classes/WellnessDimension.php');
    ...
    // Retrieve all wellness dimensions
    $wellness_dimensions = array();
    $stmt = $conn->prepare($sql_wellness_dimensions);
    $stmt->execute();
    $stmt->bind_result($dimension_id
                     , $dimension_name);
    while ($stmt->fetch()) {
        $wellness_dimensions[] = new WellnessDimension($dimension_id
                                                    , $dimension_name);
    }
    $stmt->close();
    hidden_var_dump($wellness_dimensions);
    ...
    $stmt->bind_param("ss"
                    , $dt_end->format('Y-m-d')
                    , $dt_start->format('Y-m-d'));
    $stmt->execute();
    $stmt->bind_result($habit_metric_id
                     , $metric_name
                     , $metric_id
                     , $habit_name
                     , $habit_id
                     ...
                     , $expire_datetime);
    while ($stmt->fetch()) {
        $habit_metrics[] = new HabitMetric($habit_metric_id
                                         , $metric_name
                                         , $habit_name
                                         , $habit_id
                                         , $influence
    ...
    $stmt->bind_result($habit_id
                     ...
                     , $status);
    while ($stmt->fetch()) {
        $habit_logs[] = new HabitLog($habit_id
                                   , $datetime
                                   , $status);
    }
    $stmt->close();
...
<body>
    <div id='wellness-metrics'>
        <?php   foreach($wellness_dimensions as $wd) {
                    echo "<button class='wellness-dimension' data-wd-id='$wd->id'>$wd->name</button>";
                } ?>
    </div>
                ...
                foreach($dates as $d) {
                    ...
                    || (!is_null($hm->expire_datetime) && $hm->expire_datetime->format('Y-m-d') < $d)) {
                        ...
                        foreach($habit_logs as $hl) {
                            ...
                            if ($daily_habit_completion_count > 1) {
                            ...
                            } elseif ($daily_habit_completion_count == 1) {
                                $habit_metric_row .= "<span class='daily-completions'><i class='fas fa-check'></i></span>";
                            } else {
                                $habit_metric_row .= "<span class='daily-completions'>&nbsp;</span>";
                            }

                            if ($daily_habit_start_count > 1) {
                            ...
                            } elseif ($daily_habit_start_count == 1) {
                                $habit_metric_row .= "<span class='daily-starts'><i class='fas fa-hourglass-half'></i></span>";
                            } else {
